ent-BaseLamp = фонарь
    .desc = { ent-BaseItem.desc }
ent-Lamp = лампа
    .desc = Светоизлучающее устройство.
ent-LampBanana = банана-лампа
    .desc = Светоизлучающее устройство в форме банана.
ent-LampGold = настольная лампа
    .desc = Светоизлучающее устройство, которое будет отлично смотреться на столе.
ent-LampInterrogator = лампа дознавателя
    .desc = Сверхъяркая лампа для плохого копа.
# Exodus: description follows the approved upstream power change.
ent-Floodlight = прожектор
    .desc = Столб с установленными на него мощными фонарями. Не расходует заряд, но требует установленной батареи.
ent-FloodlightBroken = сломанный прожектор
    .desc = Столб с установленными на него мощными фонарями. Он сломан.
