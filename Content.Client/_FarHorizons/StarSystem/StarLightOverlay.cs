...
public sealed class StarLightOverlay : Overlay
{
    ...
    private readonly StarSystemNebulaVisibility _nebulaVisibility; // Exodus
    ...
    private float _visibility = 1f; // Exodus
    ...
    public StarLightOverlay(
        ...
        StarSystemNebulaVisibility nebulaVisibility) // Exodus
    {
        _entMan = entMan;
        _mapMan = mapMan;
        _protoMan = protoMan;
    ...
    {
        _visibility = _nebulaVisibility.GetVisibility(args); // Exodus

        return _visibility > 0f &&
    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        _shader.SetParameter("visibility", _visibility); // Exodus
        ...
        Star star)
    {
        ...
        foreach (var grid in _grids)
        {
            ...
            var glowColor = overStar
                ? new Color(star.Color.R * lit, star.Color.G * lit, star.Color.B * lit).WithAlpha(_visibility) // Exodus
                : Color.Transparent;
        ...
        var shadowColor = comp.AmbientFloor.WithAlpha(Math.Clamp(comp.ShadowStrength, 0f, 1f) * _visibility); // Exodus
